...
open-admin-example 是一个基于 [open-admin](https://github.com/jiangood/open-admin) 框架的示例/模板项目，演示业务项目如何快速接入 open-admin 获得完整的后台管理能力。该项目的业务代码极少，核心能力由 open-admin 框架（`io.github.jiangood:open-admin`）提供。

框架源码在同级目录中，调试和定制框架行为时需要同时参考两个项目。
